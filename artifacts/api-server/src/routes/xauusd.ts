...
router.get("/xauusd/price", async (req, res) => {
  try {
    res.json(await getXauusdPrice());
  } catch (err) {
    if (err instanceof Error && err.message === "Price data unavailable") {
...
router.get("/xauusd/chart", async (req, res) => {
  try {
    ...
    res.json(await getXauusdCandles(interval));
  } catch (err) {
...
router.get("/xauusd/technicals", async (req, res) => {
  try {
    res.json(await getXauusdTechnicals());
  } catch (err) {
    if (err instanceof Error && err.message === "Not enough data") {
