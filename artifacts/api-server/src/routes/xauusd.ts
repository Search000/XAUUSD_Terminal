...
// ─── VaR / Stress Test Simulator ────────────────────────────────────────────
// Institutional desks run this against Bloomberg PORT / RiskMetrics with
// proprietary covariance data. This is the retail-free version: it derives
// volatility and worst-case drawdowns straight from public daily OHLC
// (Yahoo Finance GC=F), no paid risk-data vendor required.
let varStressCache: any = null;
let varStressCacheAt = 0;
const VAR_STRESS_TTL = 5 * 60 * 1000; // 5 min — daily data, no need to hammer the feed
...
function rollingCumReturn(returns: number[], window: number): number {
  // Worst cumulative % return over any `window`-day rolling stretch.
  let worst = 0;
  for (let i = 0; i <= returns.length - window; i++) {
    const slice = returns.slice(i, i + window);
    const cum = slice.reduce((acc, r) => (1 + acc) * (1 + r) - 1, 0);
    if (cum < worst) worst = cum;
  }
  return worst;
}

router.get("/xauusd/var-stress", async (_req, res) => {
  const now = Date.now();
  if (varStressCache && now - varStressCacheAt < VAR_STRESS_TTL) { res.json(varStressCache); return; }

  let responded = false;
  const guard = setTimeout(() => {
    if (!responded) {
      responded = true;
      if (varStressCache) res.json(varStressCache);
      else res.status(503).json({ error: "Data unavailable — try again shortly" });
    }
  }, 20_000);

  try {
    // 5y daily bars — long enough to capture the COVID crash + several
    // Fed-shock drawdowns, well within Yahoo's free chart API.
    const data = await yfChart("GC=F", "1d", "5y");
    if (responded) return;
    clearTimeout(guard);
    responded = true;
    ...
    if (candles.length < 60) {
      res.status(503).json({ error: "Not enough history" });
      return;
    }
    ...
    const parametricVar95Pct = 1.645 * dailySigma; // 95% one-tailed z-score
    const parametricVar99Pct = 2.326 * dailySigma; // 99% one-tailed z-score

    // Historical (non-parametric) VaR — actual worst tail from real data
    const sortedReturns = [...dailyReturns].sort((a, b) => a - b);
    const historicalVar95Pct = -percentile(sortedReturns, 0.05);
    const historicalVar99Pct = -percentile(sortedReturns, 0.01);

    // Stress scenarios — actual worst rolling drawdowns found in the data,
    // not hardcoded numbers. This is what a fund's stress-test grid does,
    // minus the proprietary multi-asset shock library.
    const worst1d = rollingCumReturn(dailyReturns, 1);
    const worst5d = rollingCumReturn(dailyReturns, 5);
    const worst20d = rollingCumReturn(dailyReturns, 20);

    const OZ_PER_LOT = 100; // standard XAUUSD lot

    const toUsdPerLot = (pct: number) => parseFloat((Math.abs(pct) * price * OZ_PER_LOT).toFixed(2));

    varStressCache = {
      price: parseFloat(price.toFixed(2)),
      ozPerLot: OZ_PER_LOT,
      sampleDays: dailyReturns.length,
      dailyVolPct: parseFloat((dailySigma * 100).toFixed(3)),
      var: {
        parametric95Pct: parseFloat((parametricVar95Pct * 100).toFixed(3)),
        parametric99Pct: parseFloat((parametricVar99Pct * 100).toFixed(3)),
        historical95Pct: parseFloat((historicalVar95Pct * 100).toFixed(3)),
        historical99Pct: parseFloat((historicalVar99Pct * 100).toFixed(3)),
        parametric95UsdPerLot: toUsdPerLot(parametricVar95Pct),
        parametric99UsdPerLot: toUsdPerLot(parametricVar99Pct),
        historical95UsdPerLot: toUsdPerLot(historicalVar95Pct),
        historical99UsdPerLot: toUsdPerLot(historicalVar99Pct),
      },
      stress: [
        { label: "Worst 1-Day Move (5y)", pct: parseFloat((worst1d * 100).toFixed(2)), usdPerLot: toUsdPerLot(worst1d) },
        { label: "Worst 5-Day Stretch (5y)", pct: parseFloat((worst5d * 100).toFixed(2)), usdPerLot: toUsdPerLot(worst5d) },
        { label: "Worst 20-Day Stretch (5y)", pct: parseFloat((worst20d * 100).toFixed(2)), usdPerLot: toUsdPerLot(worst20d) },
      ],
      updatedAt: now,
    };
    varStressCacheAt = now;
    res.json(varStressCache);
  } catch (err) {
    clearTimeout(guard);
    if (responded) return;
    responded = true;
    logger.error({ err }, "Failed to compute VaR/stress");
    if (varStressCache) res.json(varStressCache);
    else res.status(500).json({ error: "Failed to compute VaR/stress" });
  }
});

// ─── Options Flow: GLD options chain (Yahoo) used as a gold proxy ─────────────
// XAUUSD itself has no listed options, so GLD (SPDR Gold Shares) — the most
// liquid gold-tracking options market — is used as a proxy. Net gamma exposure
// is an estimate (assumes the standard dealer-short-calls/long-puts convention),
// not real market-maker positioning data.
let optionsFlowCache: any = null;
let optionsFlowCacheAt = 0;
const OPTIONS_FLOW_TTL = 15 * 60 * 1000; // 15 min — options data doesn't need to be tighter

async function yfOptions(symbol: string): Promise<any> {
  const hosts = [
  ...
  let lastErr: unknown;
  for (const host of hosts) {
    const url = `${host}/${encodeURIComponent(symbol)}`;
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), YF_TIMEOUT_MS);
    try {
      const res = await fetch(url, {
      ...
      clearTimeout(timer);
      if (res.status === 429) { lastErr = new Error(`Options feed rate-limited (429) for ${symbol}`); continue; }
      if (!res.ok) throw new Error(`Options feed error: ${res.status} for ${symbol}`);
      return await res.json();
    } catch (err) {
      clearTimeout(timer);
      lastErr = err;
...
// Black-Scholes gamma — same for calls and puts at a given strike/IV/T.
function bsGamma(spot: number, strike: number, iv: number, tYears: number, r = 0.05): number {
  if (spot <= 0 || strike <= 0 || iv <= 0 || tYears <= 0) return 0;
  const d1 = (Math.log(spot / strike) + (r + (iv * iv) / 2) * tYears) / (iv * Math.sqrt(tYears));
  return stdNormPdf(d1) / (spot * iv * Math.sqrt(tYears));
}

router.get("/xauusd/options-flow", async (_req, res) => {
  const now = Date.now();
  if (optionsFlowCache && now - optionsFlowCacheAt < OPTIONS_FLOW_TTL) { res.json(optionsFlowCache); return; }

  let responded = false;
  const guard = setTimeout(() => {
    if (!responded) {
      responded = true;
      if (optionsFlowCache) res.json(optionsFlowCache);
      else res.status(503).json({ error: "Data unavailable — try again shortly" });
    }
  }, 20_000);

  try {
    const data = await yfOptions("GLD");
    if (responded) return;
    clearTimeout(guard);
    responded = true;
    ...
    const expirationSec: number = opt?.expirationDate ?? 0;

    if (!spot || (calls.length === 0 && puts.length === 0)) {
      res.status(503).json({ error: "No options data available" });
      return;
    }

    const tYears = Math.max((expirationSec * 1000 - now) / (365 * 24 * 60 * 60 * 1000), 1 / 365);
    ...
    const putCallVolRatio = totalCallVol > 0 ? totalPutVol / totalCallVol : 0;
    const putCallOiRatio = totalCallOi > 0 ? totalPutOi / totalCallOi : 0;

    // Unusual activity: near-the-money strikes with high volume relative to
    // open interest — a spike in same-day volume vs standing OI is the
    // classic "someone just put on a large fresh position" signal.
    const nearMoney = (strike: number) => Math.abs(strike - spot) / spot <= 0.15;
    const withRatio = (list: any[], type: "CALL" | "PUT") =>
      list
        .filter((o) => nearMoney(o.strike) && (o.volume || 0) > 0)
        .map((o) => ({
          type,
          strike: o.strike,
          volume: o.volume || 0,
          openInterest: o.openInterest || 0,
          volOiRatio: o.openInterest > 0 ? (o.volume || 0) / o.openInterest : (o.volume || 0),
          iv: o.impliedVolatility ?? null,
          inTheMoney: !!o.inTheMoney,
        }));

    const unusual = [...withRatio(calls, "CALL"), ...withRatio(puts, "PUT")]
      .sort((a, b) => b.volOiRatio - a.volOiRatio)
      .slice(0, 6)
      .map((o) => ({
        ...o,
        volOiRatio: parseFloat(o.volOiRatio.toFixed(2)),
        signal: o.type === "CALL" ? "bullish" : "bearish",
      }));

    // Net gamma exposure estimate — standard convention assumes dealers are
    // net short calls and net long puts. Positive net = dealers buy dips /
    // sell rips (stabilizing); negative = dealers chase the move (amplifying).
    let callGex = 0;
    let putGex = 0;
    for (const c of calls) {
      ...
      if (!iv || oi <= 0) continue;
      const gamma = bsGamma(spot, c.strike, iv, tYears);
    ...
    for (const p of puts) {
      ...
      if (!iv || oi <= 0) continue;
      const gamma = bsGamma(spot, p.strike, iv, tYears);
      putGex += gamma * oi * 100 * spot * spot * 0.01;
    }
    const netGex = -callGex + putGex; // dealer perspective

    optionsFlowCache = {
      underlying: "GLD",
      spot: parseFloat(spot.toFixed(2)),
      expiration: expirationSec * 1000,
      totalCallVol,
      totalPutVol,
      totalCallOi,
      totalPutOi,
      putCallVolRatio: parseFloat(putCallVolRatio.toFixed(2)),
      putCallOiRatio: parseFloat(putCallOiRatio.toFixed(2)),
      unusual,
      gamma: {
        netGexUsd: Math.round(netGex),
        regime: netGex >= 0 ? "positive" : "negative",
      },
      updatedAt: now,
    };
    optionsFlowCacheAt = now;
    res.json(optionsFlowCache);
  } catch (err) {
    clearTimeout(guard);
    if (responded) return;
    responded = true;
    logger.error({ err }, "Failed to compute options flow");
    if (optionsFlowCache) res.json(optionsFlowCache);
    else res.status(500).json({ error: "Failed to compute options flow" });
  }
});
